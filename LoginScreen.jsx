        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Enter your email"
            style={{
              ...
              outline:'none',transition:'border-color 0.2s',opacity:show?1:0,
              transform:show?'translateY(0)':'translateY(8px)',transitionDelay:'0.08s'
          ...
          <div style={{position:'relative',opacity:show?1:0,transform:show?'translateY(0)':'translateY(8px)',transition:'all 0.3s ease 0.16s'}}>
            <input value={pass} onChange={e=>setPass(e.target.value)} placeholder="Enter your password"
              ...
              style={{
                width:'100%',padding:'12px 16px',paddingRight:44,borderRadius:12,border:'0.5px solid var(--border)',
                fontSize:14,fontFamily:'inherit',background:'var(--bg)',color:'var(--text)',outline:'none',transition:'border-color 0.2s'
              }} onFocus={e=>e.target.style.borderColor='#534AB7'} onBlur={e=>e.target.style.borderColor='var(--border)'}/>
          ...
          <button onClick={handleSign} disabled={loading} style={{
            ...
            transitionDelay:'0.24s',display:'flex',alignItems:'center',justifyContent:'center',gap:8
          }}>
          ...
          <button onClick={onLogin} style={{
            ...
            fontFamily:'inherit',cursor:'pointer',padding:8,transition:'color 0.2s'
          }}>Continue as Guest</button>
        </div>

        <div style={{textAlign:'center',marginTop:16,fontSize:13,color:'var(--text-muted)'}}>
          Don't have an account? <span style={{color:'#534AB7',cursor:'pointer',fontWeight:500}} onClick={onLogin}>Get started</span>
        </div>

        {/* Demo credentials */}
        <div style={{
          marginTop:20,padding:12,borderRadius:8,background:'var(--bg)',
          border:'0.5px solid var(--border)',textAlign:'center',fontSize:12,color:'var(--text-muted)'
        }}>
