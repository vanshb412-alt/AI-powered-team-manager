  ...
  return (
    <div style={{maxWidth:960,margin:'0 auto',padding:'32px 24px'}}>
      ...
      <div style={{marginBottom:24}}>
        ...
        <div style={{fontSize:13,color:'var(--text-muted)'}}>8 unlocked · 4 locked</div>
      </div>
      ...
      <div style={{
        ...
        <div style={{fontSize:16,fontWeight:600}}>Level 7 — Sprint Warrior</div>
        <div style={{background:'rgba(255,255,255,0.2)',borderRadius:4,height:8,marginTop:12,overflow:'hidden'}}>
          <div style={{height:'100%',background:'white',borderRadius:4,width:show?'80%':'0%',transition:'width 0.8s ease-out 0.3s'}}/>
        </div>
        <div style={{fontSize:12,opacity:0.8,marginTop:8}}>2400 / 3000 XP · 600 XP to Level 8 — Task Master</div>
      </div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:24,flexWrap:'wrap'}}>
        {cats.map(c=>(
          <button key={c} onClick={()=>setFilter(c)} style={{
            ...
            color:filter===c?'white':'var(--primary)',
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
        {filtered.map((a,i)=>(
          <div key={a.name} className="card-inner" style={{
            padding:20,opacity:show?1:0,
            ...
            <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:12}}>
              <div style={{
                ...
                background:a.unlocked?`${tierColors[a.tier]}20`:'var(--bg)',
                display:'flex',alignItems:'center',justifyContent:'center',fontSize:22
              }}>{tierIcons[a.tier]}</div>
              <div style={{flex:1}}>
            ...
            {a.unlocked ? (
              <>
                <div style={{fontSize:11,color:'var(--text-muted)',marginBottom:8}}>Unlocked {a.date}</div>
                <div style={{height:4,background:'var(--border)',borderRadius:2,overflow:'hidden'}}>
                  <div style={{height:'100%',background:tierColors[a.tier],borderRadius:2,
                    width:show?'100%':'0%',transition:`width 0.6s ease-out ${0.4+i*0.06}s`}}/>
                </div>
              </>
            ) : (
              <>
                <div style={{fontSize:12,color:'var(--text-muted)',marginBottom:8}}>{a.progress}</div>
                <div style={{height:4,background:'var(--border)',borderRadius:2,overflow:'hidden'}}>
                  <div style={{height:'100%',background:'var(--text-muted)',borderRadius:2,
                    width:show?`${a.pct}%`:'0%',transition:`width 0.6s ease-out ${0.4+i*0.06}s`}}/>
                </div>
            ...
            <div style={{marginTop:8}}>
              <span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:6,
                background:'var(--pill-bg)',color:'var(--primary)'}}>{a.cat}</span>
            </div>
          </div>
        ))}
      </div>
