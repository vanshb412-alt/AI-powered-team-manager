...
function genCode() {
  ...
  let r = 'SYN-';
  for (let i = 0; i < 3; i++) r += c[Math.floor(Math.random() * c.length)];
  return r;
}
  ...
  return (
    <div style={{ opacity: show ? 1 : 0, transform: show ? 'translateY(0)' : 'translateY(8px)', transition: `all 0.3s ease ${delay}s` }}>
      ...
      <input value={value} onChange={e => onChange(mono ? e.target.value.toUpperCase().slice(0, 7) : e.target.value)}
        ...
        style={{
          width: '100%', padding: '12px 16px', borderRadius: 12, border: '0.5px solid var(--border)',
          fontSize: 14, fontFamily: mono ? 'monospace' : 'inherit', background: 'var(--bg)',
          color: 'var(--text)', outline: 'none', transition: 'border-color 0.2s', letterSpacing: mono ? 2 : 0
        }}
...
export default function OnboardingScreen({ role, onComplete }) {
  ...
  const canSubmit = isLeader
    ? name && roleTitle && teamName && projectName
    : name && roleTitle && joinCode;

  const handleSubmit = () => {
    if (!canSubmit || loading) return;
    setLoading(true);
    const code = isLeader ? genCode() : '';
    setTimeout(() => {
      setLoading(false);
  ...
  const handleFinish = () => {
    onComplete({
      userName: name, userRoleTitle: roleTitle,
      teamName: isLeader ? teamName : 'Team Synapse',
      projectName: isLeader ? projectName : 'Mobile App Redesign',
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 24 }}>
      <div style={{
        ...
        padding: 40, maxWidth: 480, width: '100%', boxShadow: '0 2px 12px rgba(83,74,183,0.08)',
        ...
        {!success ? (
          <>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>
              {isLeader ? 'You can always change these later' : 'Enter the invite code your team leader shared'}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button onClick={handleSubmit} disabled={!canSubmit || loading} style={{
                ...
                background: canSubmit ? '#534AB7' : 'var(--border)', color: canSubmit ? 'white' : 'var(--text-muted)',
                fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: canSubmit ? 'pointer' : 'not-allowed',
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 16, fontWeight: 600, color: '#1D9E75', marginTop: 16 }}>
              {isLeader ? 'Team created successfully!' : "You've joined the team!"}
            </h2>
            {isLeader ? (
              <>
                ...
                <div style={{
                  ...
                  margin: '16px auto', padding: '12px 24px', border: '1.5px solid var(--border)',
                  borderRadius: 12, background: 'var(--bg)', maxWidth: 200,
                  transform: success ? 'scale(1)' : 'scale(0.8)', opacity: success ? 1 : 0,
                  transition: 'all 0.3s ease 0.3s'
                }}>
                  <span style={{ fontSize: 32, fontWeight: 600, color: '#534AB7', fontFamily: 'monospace', letterSpacing: 3 }}>{generatedCode}</span>
                  <button onClick={copyCode} style={{
                    background: 'none', border: 'none', cursor: 'pointer', fontSize: 16,
                    color: copied ? '#1D9E75' : 'var(--text-muted)', transition: 'color 0.2s'
                  }}>{copied ? '✓' : '📋'}</button>
                </div>
              </>
            ) : (
              <p style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 12 }}>Welcome to Team Synapse</p>
            )}
            <button onClick={handleFinish} style={{
              width: '100%', padding: 14, borderRadius: 12, border: 'none', marginTop: 20,
              background: '#534AB7', color: 'white', fontSize: 14, fontWeight: 600,
              fontFamily: 'inherit', cursor: 'pointer', transition: 'all 0.2s'
            }}>{isLeader ? 'Go to Dashboard →' : "Let's get you set up →"}</button>
          </div>
        )}
      </div>
    </div>
